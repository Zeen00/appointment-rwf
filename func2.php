...
$CheckSQL = "SELECT * FROM patcheck WHERE rwfid='$rwfid' AND dob='$dob'";

// Executing userid Check MySQL Query.
...
if(isset($check)){
    ...
    $CheckSQLL = "SELECT * FROM patreg WHERE rwfid='$rwfid' AND dob='$dob'";
    $checkk = mysqli_fetch_array(mysqli_query($con,$CheckSQLL));
    ...
    //     // If the record inserted successfully then show the message.
		...
		// $json = json_encode($MSGG);
		 
		// // Echo the message.
		//  echo $json ; 
    echo "<script>alert('User Already Exists! please login');document.location='index.php'</script>";
    }
    else{ 
 ...
 if($password==$cpassword){
  global $mysqli; 
 $query="insert into patreg(fname,lname,gender,email,contact,password,cpassword,rwfid,dob) values ('$fname','$lname','$gender','$email','$contact','$password','$cpassword','$rwfid','$dob');";
  ...
  // 	 $no_userExist = 'User ID Does not Exist, Please Try Again With New User ID..!';
	 
	//  // Converting the message into JSON format.
	// $no_userexistJSON = json_encode($no_userExist);
	 
	// // Echo the message on Screen.
	//  echo $no_userexistJSON ; 
  echo "<script>alert('User ID Does not Exist, Please Try Again With New User ID..!');document.location='index.php'</script>";
  //redirect('index.php');
